...
DO NOT MAKE A NEW WINDOW IN YOUR FUNCTIONS IT IS PROVIDED FOR YOU
"""
...
def draw_triangle(t, s, c):
    ...
    """
    # Assert the preconditions
    assert is_valid_turtlemode(t), report_error('t is not a valid turtle mode', t)
    assert is_valid_length(s), report_error('s is not a valid side length', s)
    assert is_valid_color(c), report_error('c i not a valid color', c)
...
def draw_spiral(w, side, ang, n, sp):
    ...
    assert is_valid_speed(sp), report_error('sp is not a valid turtle speed', sp)
...
def draw_spiral_helper(t, side, ang, n, sp):
    ...
    drawmode = t.drawmode
    visible = t.visible
    #storing initial value in variable
...
def multi_polygons(w, side, k, n, sp):
    ...
    assert is_window(w), report_error('w is not a valid window',w)
    assert is_valid_length(side), report_error('side is not a valid side length',side)
    assert is_number(k), report_error('k is not a number', k)
    assert is_number(n), report_error('n is not a number', n)
    assert is_valid_speed(sp), report_error('sp is not a valid turtle speed', sp)
...
def multi_polygons_helper(t, side, k, n, sp):
    ...
    assert is_valid_turtlemode(t), report_error('Invalid turtle mode', t)
    assert is_valid_length(side), report_error('side is not a valid length',side)
    assert is_number(k), report_error('k is not a valid number', k)
...
def triangle_helper(p, x, y, side, d):
    ...
    if d != 0:
        triangle_helper(p, x-side/4.0, y, side/2.0, d-1)
        triangle_helper(p, x+side/4.0, y, side/2.0, d-1)
        triangle_helper(p, x, y+side*(math.sqrt(3.0))/4.0, side/2.0, d-1)


        #triangle_helper(p, x, y, side/2.0, d-1)
        #triangle_helper(p, x+side/2.0, y, side/2.0, d-1)
        #triangle_helper(p, x+side/4.0, y+(math.sqrt(3)/2*side/2.0), side/2.0, d-1)
...
def cantor(w, side, hght, d, sp):
    ...
    x = 0
    y = 0
    cantor_helper(p, x, y, side, hght, d)
    p.visible = False
    p.flush()
...
def island(w, side, d, sp):
    ...
    t.move(0.0+side/2.0,0.0-side/2.0)
    t.left(90)
...
def island_edge(t, side, d):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_valid_turtlemode(t), report_error('Invalid turtle mode', t)
    assert is_valid_length(side), report_error('Invalid side length', side)
    assert is_valid_depth(d), report_error('Invalid depth', d)
