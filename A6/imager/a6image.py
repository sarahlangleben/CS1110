...
Sarah Langleben, sml343 and Nicole Yatksar, ny73
11/12/2020
"""
...
# TASK 0: IMPLEMENT THIS HELPER : DONE
...
class Image(object):
    ...
    def __setitem__(self, pos, pixel):
        ...
        assert len(pixel) == 3
        assert type(self._data[pos]) == tuple
        assert type(pixel[0]) == int
        assert pixel[0] >= 0 and pixel[0] <= 255
        assert type(pixel[1]) == int
        assert pixel[1] >= 0 and pixel[1] <= 255
        assert type(pixel[2]) == int
        assert pixel[2] >= 0 and pixel[2] <= 255
    ...
    def getPixel(self, row, col):
        ...
        return self._data[(row * self._width) + col]
    ...
    def __str__(self):
        ...
        for i in range(len(self._data)):
            if count == 0:
                answer = answer + '['
            if count > 0 and count < self._width:
                answer = answer + ', '
            if count == self._width:
                ...
                continue
            count = count + 1
            answer = answer + str(self._data[i])
        answer = answer + ']]'
        return answer
    ...
    def swapPixels(self, row1, col1, row2, col2):
        ...
        first = self.getPixel(row1, col1)
        self.setPixel(row1, col1, self.getPixel(row2, col2))
        self.setPixel(row2, col2, first)
